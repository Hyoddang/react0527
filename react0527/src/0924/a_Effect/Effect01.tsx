...
function VideoPlayer({ src, isPlaying}: VideoPlayerProps) {
  ...
  // DOM 업데이트를 Effect로 감싸면 React가 먼저 화면을 업데이트 한다.
  // >> 그 다음 Effect가 실행.
  useEffect(() => {
    ...
    if (ref.current) {
      if (isPlaying) { //! isPlaying을 사용했기 때문에
        ...
        ref.current.pause(); // isPlaying이 false일 때 비디오 일시정지
      }
    }
    // React에게 불필요하게 Effect를 다시 실행하는 것을 방지하기 위해서는
    // useEffect 호출의 두 번째 인자로 의존성 배열을 지정
    //! 종속성이 지정되어야 한다.
  }, [isPlaying, ref]); // isPlaying이 변경될 때마다 Effect가 실행
...
export default function Effect01() {
  ...
  // useEffect 훅을 React에서 가져온 뒤
  // 컴포넌트의 최상위에서 호출하여 Effect안에 코드 작성
  useEffect(() => {
    // 이 코드는 '모든'렌더링 후에 실행
    // > 컴포넌트가 렌더링 될 때마다 React는 화면을 업데이트 하고
    //   useEffect내의 코드를 실행
    //   useEffect는 렌더링이 화면에 반영될 때까지 코드 실행을 '지연'
  })
  ...
  return (
    ...
    {/* 버튼 클릭 시 isPlaying 상태를 변경 */}
      ...
      <VideoPlayer 
      isPlaying={isPlaying}
      src='https://cdn.pixabay.com/vimeo/510850921/64767.mp4?width=1280&hash=8b045ea12e4d8c14c795c159d09ff6e36c22a47b'
      />
    </>
  )
}
