...
//! 상호작용
// 상태(시간이 지나면서 변하는 데이터)를 추가하고 필요에 따라 업데이트 사용자의 입력에 응답하여 변화되는 업데이트를 관리

//! 1. 이벤트에 응답
// 이벤트 핸들러는 클릭, 호버, 폼 입력 등에 포커스 하는 등의 사용자 상호작용에 반응해서 함수를 커스터마이징 할 수 있다.

// button 내장 컴포넌트(태그)는 onClick과 같은 내장 브라우저 이벤트만을 지원

// 개발자가 직접 만든 컴포넌트는 이벤트 핸들러 prop에 원하는 이벤트 핸들러 작성 가능
...
export default function Interactivity01() {
  return (
    ...
    onUploadImage={() => alert('Uploading')}></Toolbar>
  )
}
