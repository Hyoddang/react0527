...
export default function Interactivity02() {
  ...
  // 함수 전달(x): onClick={handleClick()}
  // 함수명 뒤에 따라오는 소괄호()는 어떤 클릭도 없이 렌더링 중에 함수를 즉시 실행

  // 2. 인라인으로 코드를 작성하는 경우 전달 형식으로 작성해야 한다.
  // 함수 전달(o): <button onClick{() => alert('...')}> // 입력함수로 래핑
