...
// 새로운 할 일을 등록 할 수 있는 컴포넌트 생성

// useState를 사용하여 토글 할 수 있는 open 값 관리
// : 해당 값이 true일 때: 아이콘을 45도 돌려 +를 x 모양으로 보여지게 나타냄.
//   버튼 색상도 빨간색으로 변경
//   할 일을 입력할 수 있는 폼을 보여줌
...
const CircleButton = styled.button<CircleButtonProps>`
  ...
  */
  ...
  ${props =>
    props.open && //props.open이 true일 경우 아래 css적용
    css`
      background: #ff6b6b;
      &:hover {
        background: #ff8787;
...
const InsertFormPositional = styled.div`
  width: 100%;
  bottom: 0;
  left: 0%;
  position: absolute;
...
function TodoCreate() {
  ...
  return (
    <>
      {open && (
        <InsertFormPositional>
          <InsertForm onSubmit={onSubmit}>
            <Input autoFocus placeholder='할 일 입력 후, Enter를 누르세요.' onChange={onChange} value={value} />
          </InsertForm>
        </InsertFormPositional>
      )}
      {/* 
        CircleButton을 클릭하면 onToggle 함수가 실행,
        open 상태가 토글 
      */}
