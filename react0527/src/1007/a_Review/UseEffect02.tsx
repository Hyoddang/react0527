...
export default function UseEffect02() {
  ...
  const usersData: User[] = [
    {'id': 1, 'name': 'Lee', 'email': 'asd@email.com'},
    {'id': 2, 'name': 'Yoon', 'email': 'qwe@email.com'},
  ]
  ...
  useEffect(() => {
    ...
    .then((response) => response.json()) // API응답을 JSON 형태로 파싱
  ...
  const handleIdChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    // 숫자로 변환하여 상태를 업데이트
    setUserId(Number(e.target.value)); // 타입변환
  }
